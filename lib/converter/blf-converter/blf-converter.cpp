...
bool BLF_write(Vector::BLF::ObjectType module, const char *filepath, const json &blf_records)
{
    ...
    try
    {
        ...
        std::for_each(blf_records.begin(), blf_records.end(), [&](const json &record) {

            if (module == Vector::BLF::ObjectType::CAN_MESSAGE)
            {
                Vector::BLF::CanMessage *obj = new Vector::BLF::CanMessage();

                obj->channel = record["CHANNEL"];
                obj->objectFlags = Vector::BLF::ObjectHeader::ObjectFlags::TimeOneNans;
                obj->objectTimeStamp = record["DELAY"].get<double>() * 100000;
                obj->id = record["MSG"]["ID"];
                obj->flags &= 0xFE;
                obj->flags |= record["DIR"] == "Tx" ? 1 : 0;
                obj->dlc = record["MSG"]["LEN"];
                std::copy(record["MSG"]["DATA"].begin(), record["MSG"]["DATA"].end(), obj->data.begin());

                file.write((Vector::BLF::ObjectHeaderBase *)obj);
            }
            else if (module == Vector::BLF::ObjectType::LIN_MESSAGE)
            {
                Vector::BLF::LinMessage *obj = new Vector::BLF::LinMessage();
    ...
    {
        std::cout << "EXCEPTION in BLF_write():" << std::endl;
        return false;
    }
}

bool handleCanMessage(Vector::BLF::CanMessage *message, CAN_LIN_RECORD_T &record)
{
    ...
    record.msg.DATA.resize(record.msg.LEN);
    std::copy(message->data.begin(), message->data.end(), record.msg.DATA.begin());

    return true;
}

bool handleLinMessage(Vector::BLF::LinMessage *message, CAN_LIN_RECORD_T &record)
{
    record.channel = message->channel;
    record.delay = message->objectFlags == Vector::BLF::ObjectHeader::ObjectFlags::TimeOneNans ? (double)message->objectTimeStamp / 100000.0 : message->objectTimeStamp;
...
int BLF_read(Vector::BLF::ObjectType module, const char *filepath, json &records)
{
    ...
    std::cout << moduleStr << " message count: " << std::dec << count << std::endl;
